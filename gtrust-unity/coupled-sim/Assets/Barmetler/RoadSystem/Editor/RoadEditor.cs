...
namespace Barmetler.RoadSystem
{
    ...
    {
        private readonly Dictionary<int, Quaternion> initialRotations = new();

        private readonly Dictionary<KeyCode, bool> wasDown = new();
        private static readonly HashSet<RoadEditor> ActiveEditors = new();
        ...
        private static Rect windowRect = new(10000, 10000, 300, 300);
        ...
        {
            return ActiveEditors.FirstOrDefault(e => ((Road) e.target).gameObject == gameObject);
        }
        ...
        private void OnSceneGUI()
        {
            ...
            if (selectedAnchorPoint >= road.NumPoints)
            {
                selectedAnchorPoint = road.NumPoints - 1;
            }
        ...
        private void DrawInfo(int controlID)
        {
            ...
            var points = road.GetEvenlySpacedPoints(1).Select(e => e.ToWorldSpace(road.transform)).ToArray();
                ...
                switch (Tools.current)
                {
                    ...
                    case Tool.Scale:
                        if (selectedAnchorPoint != -1)
                        {
                            ...
                            for (var i = -1; i <= 1; i += 2)
                            {
                                ...
                                {
                                    var fmh_246_50_638092194452167027 = Quaternion.identity;

                                    var nPos = Handles.FreeMoveHandle(hPos,
                                    ...
                                    if (hPos != nPos)
                                    {
                                        ...
                                        if (i == -1)
                                        {
                                            dot = Mathf.Min(dot, -0.1f);
                                        }
                                        else
                                        {
                                            dot = Mathf.Max(dot, 0.1f);
                                        }
                    ...
                    case Tool.Rect:
                        ...
                        foreach (var p in points)
                        {
                            ...
                            {
                                var fmh_298_87_638092194452208448 = Quaternion.identity;
        ...
        private void GUIAddOrRemovePoints(int controlID)
        {
            ...
            {
                var minDist = float.PositiveInfinity;
                var selectedEndpoint = (isValid: false, isStart: false, position: Vector3.zero, index: 0);

                foreach (var isStart in new[] {true, false})
                {
                    if ((isStart && road.start) || (!isStart && road.end))
                    {
                        continue;
                    }
        ...
        private void BoolField(string label, bool value, Action<bool> setter, Object obj, bool endHorizontal = true)
        {
