...
class Analysis:
    ...
    def plot_kp(self, df, conf_interval=None, save_file=True, save_final=True):
        ...
        fig = make_subplots(rows=len(conditions), cols=1, subplot_titles=conditions, shared_xaxes=True)
        ...
        for i, condition in enumerate(conditions):
            ...
            for role in roles:
                df_role = df_condition[df_condition['role'] == role]
                kp_data = np.zeros(len(times))
                count = 0
                
                for data in df_role['EmperorsRating_UnixTime'].dropna():
                    kp_time = (int(data) - min_time) / 1000  # Convert to seconds, start from 0
                    closest_idx = np.argmin(np.abs(times - kp_time))
                    kp_data[closest_idx] += 1
                    count += 1
